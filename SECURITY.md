...
| Battery | IOKit power-source info, plus the system's own Low Power Mode flag (`ProcessInfo`) | Level, whether it is charging / held / full, time remaining or time to full, and whether Low Power Mode is on. Read-only — macOS offers no public way to *switch* Low Power Mode, so the panel's Low Power Mode row opens System Settings at the Battery pane and you make the change there. |
